...
complete -c git-diff-files -s p -s u -l patch -d 'Generate patch (see the section called "GENERATING PATCH TEXT WITH -P")'
complete -c git-diff-files -s s -l no-patch -d 'Suppress all output from the diff machinery'
...
complete -c git-diff-files -s c -l cc -d 'This compares stage 2 (our branch), stage 3 (their branch), and the working t…'
complete -c git-diff-files -s q -d 'Remain silent even for nonexistent files'
complete -c git-diff-files -l diff-algorithm
complete -c git-diff-files -l stat-graph-width -d 'or by setting diff. statGraphWidth=<width>.  Using'

...
complete -c git-diff-index -s p -s u -l patch -d 'Generate patch (see the section called "GENERATING PATCH TEXT WITH -P")'
...
complete -c git-diff-index -l stat-graph-width -d 'or by setting diff. statGraphWidth=<width>.  Using'

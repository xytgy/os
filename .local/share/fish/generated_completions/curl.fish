...
complete -c curl -l ca-native -d '(TLS) Tells curl to use the CA store from the native operating system to veri…'
...
complete -c curl -l dns-ipv4-addr -d '(DNS) Tell curl to bind to a specific IP address when making IPv4 DNS request…'
complete -c curl -l dns-ipv6-addr -d '(DNS) Tell curl to bind to a specific IP address when making IPv6 DNS request…'
...
complete -c curl -s G -l get -d 'When used, this option makes all data specified with -d, --data, --data-binar…'
...
complete -c curl -l ipfs-gateway -d 'Specify which gateway to use for IPFS and IPNS URLs'
complete -c curl -s 4 -l ipv4 -d 'This option tells curl to use IPv4 addresses only when resolving host names, …'
complete -c curl -s 6 -l ipv6 -d 'This option tells curl to use IPv6 addresses only when resolving host names, …'
...
complete -c curl -s l -l list-only -d '(FTP POP3 SFTP) (FTP) When listing an FTP directory, this switch forces a nam…'
...
complete -c curl -l proxy-ca-native -d '(TLS) Tells curl to use the CA store from the native operating system to veri…'
...
complete -c curl -l proxy-http2 -d '(HTTP) Tells curl to try negotiate HTTP version 2 with an HTTPS proxy'
...
complete -c curl -l ssl-auto-client-cert -d '(Schannel) Tell libcurl to automatically locate and use a client certificate …'
...
complete -c curl -l tftp-blksize -d '(TFTP) Set the TFTP BLKSIZE option (must be >512)'
...
complete -c curl -l tlsv1 -d '(TLS) Tells curl to use at least TLS version 1'

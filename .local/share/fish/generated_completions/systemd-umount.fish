...
complete -c systemd-umount -s l -l full -d 'Do not ellipsize the output when --list is specified. sp Added in version 245'
...
complete -c systemd-umount -l options -s o -d 'Additional mount options for the mount point. sp Added in version 232'
...
complete -c systemd-umount -s A -d 'Equivalent to --automount=yes. sp Added in version 232'
...
complete -c systemd-umount -s T -l tmpfs -d 'Create and mount a new tmpfs file system on WHERE, with an optional NAME that…'

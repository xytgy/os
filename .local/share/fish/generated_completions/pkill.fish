...
complete -c pkill -s A -l ignore-ancestors -d 'Ignore all ancestors of  pgrep ,  pkill , or  pidwait '
complete -c pkill -s H -l require-handler -d 'Only match processes with a userspace signal handler present for the signal t…'
complete -c pkill -l cgroup -d 'Match on provided control group (cgroup) v2 name.  See  cgroups (8)'
...
complete -c pkill -s q -l queue -d 'Use  sigqueue (3) rather than  kill (2) and the value argument is used to spe…'

...
complete -c git-diff-tree -s p -s u -l patch -d 'Generate patch (see the section called "GENERATING PATCH TEXT WITH -P")'
...
complete -c git-diff-tree -l show-notes-by-default -d 'Show the default notes unless options for displaying specific notes are given'
...
complete -c git-diff-tree -l stat-graph-width -d 'or by setting diff. statGraphWidth=<width>.  Using'

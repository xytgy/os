...
complete -c git-diff -s p -s u -l patch -d 'Generate patch (see the section called "GENERATING PATCH TEXT WITH -P")'
...
complete -c git-diff -l stat-graph-width -d 'or by setting diff. statGraphWidth=<width>.  Using'

...
complete -c infocmp -s I -d 'If more than one terminal-type is specified, the -d option will be assumed'
complete -c infocmp -s d -d 'produces a list of each capability that is different between two entries'
complete -c infocmp -o 'I	use'
complete -c infocmp -o 'L	use'
complete -c infocmp -o 'r	with' -s C
complete -c infocmp -o 'K	with' -d 'If no terminal-types are given, the environment variable TERM will be used fo…'
...
complete -c infocmp -s r -d 'sets the subset to all capabilities'
...
complete -c infocmp -s x -d 'print information for user-defined capabilities (see %user_caps(5)'

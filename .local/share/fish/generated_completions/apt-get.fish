...
complete -c apt-get -s S -l snapshot -d 'This option controls the snapshot chosen for archives with Snapshot: enable i…'
...
complete -c apt-get -l allow-insecure-repositories -d 'Allow the update command to acquire unverifiable data from configured sources'
...
complete -c apt-get -s U -l update -d 'Run the update command before the specified command'

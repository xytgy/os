...
complete -c ld-linux -l argv0 -d 'Set  argv[0] to the value  string before running the program'
complete -c ld-linux -l audit -d 'Use objects named in  list as auditors'
complete -c ld-linux -l glibc-hwcaps-mask -d 'only search built-in subdirectories if in R list '
complete -c ld-linux -l glibc-hwcaps-prepend -d 'Search glibc-hwcaps subdirectories in R list '
...
complete -c ld-linux -l list-diagnostics -d 'Print system diagnostic information in a machine-readable format, such as som…'
...
complete -c ld-linux -l list-tunables -d 'Print the names and values of all tunables, along with the minimum and maximu…'
